...
select doc_id, doc_name, doc_birth, dep_no, dep_name from `Doctors`
join `Departments` using (dep_no);

# 문제 2

select nur_id, nur_name, nur_birth, dep_no, dep_name from `Nurses`
join `Departments` using (dep_no);

# 문제 3

select pat_id, pat_name, pat_jumin, pat_phone, doc_name, nur_name from `Patients`
join `Doctors` using (doc_id)
join `Nurses` using (nur_id);

# 문제 4

select pat_name, doc_name, treat_contents, chart_contents, treat_datetime from `Treatments`
join `Doctors` using (doc_id)
...
select treat_no, pat_name, doc_name, treat_contents, chart_contents, treat_datetime from `Treatments`
join `Doctors` using (doc_id)
join `Patients` using (pat_id)
join `Charts` using (treat_no)
where dep_no = 103;
...
select pat_name, pat_jumin from `Patients`
join `Treatments` using (pat_id)
where year(curdate()) - concat('19', substring(pat_jumin, 1, 2)) +1 >= 30 and year(curdate()) - concat('19', substring(pat_jumin, 1, 2)) + 1 < 40;
...
# 문제 9------------------- group_concat 과 separator 사용

select group_concat(pat_name separator '^') as `환자` from `Patients`
join `Nurses` using (nur_id)
where nur_name = '김태희';

# 문제 10

select nur_id, nur_name, count(pat_id) from `Nurses`
